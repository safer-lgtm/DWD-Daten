...
average_state <- function(connection, tbl_name, parameter){
    query <- dbGetQuery(con, 
    'SELECT g."state" "Bundesland", 
    avg(w.' %+% '"' %+% parameter %+% '"' %+% ') "Durchschnitt" 
    FROM ' %+% tbl_name %+% ' w
...
durchschnitt <- average(con, 'temperatur', 'V_TE005M.Erdbodentemperatur_005cm', '1951-01-01', '1951-02-01')
test <- average_state(con, 'temperatur', 'V_TE005M.Erdbodentemperatur_005cm')
...
p + geom_sf(data = germany_shp.df, aes(fill=Durchschnitt)) +
    ...
    ggtitle('Durchschnitt im Monat 1951-01') 
