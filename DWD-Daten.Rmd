...
durchschnitt <- average(con, 'temperatur', 'V_TE005M.Erdbodentemperatur_005cm', '1951-04-01', '1951-05-01')

# Postgres-Verbindung schließen
#dbDisconnect(con)

# Verbinden der Shapefile und die Durchschnitte per Bundesland
germany_shp.df <- merge(fortify(germany_shp), as.data.frame(durchschnitt), by.x="NAME_1", by.y="state")

# Um später Name der Bundesläner zu verwalten
...
p1 + geom_sf(data=germany_shp.df, aes(fill=Durchschnitt)) +
        ...
        ggtitle('Durchschnitt im Jahr-Monat 1951-01') 

# Speicherung von Plot
ggsave(p2, file = "map1.png", width = 6, height = 4.5)
